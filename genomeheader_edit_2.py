...
def extract_fields(dirs: str):
    output = "refseqAccession_genbankAccession.txt"
    ...
    for dir_ in dirs:
        ...
        with open(jsonl_input, 'r') as f_in, open(output_path, 'w') as f_out:
            for line in f_in:
                ...
                if refseqAccession is not None or genbankAccession is not None:
        ...
        refseq_genbank_file = output_path
        manipulate_fna(dir_, refseq_genbank_file)

def manipulate_fna(dir_: str, refseq_genbank_file: str):
    ...
    # Read the refseqAccession_genbankAccession.txt file into a dictionary
    refseq_genbank = {}
    with open(refseq_genbank_file, 'r') as f:
        for line in f:
            parts = line.rstrip().split('\t')
            if len(parts) == 2:
                refseqAccession, genbankAccession = parts
                refseq_genbank[refseqAccession] = genbankAccession
    ...
    with open(fna_file, 'r') as f_in, open(fna_file + ".modified", 'w') as f_out:
        for line in f_in:
            if line.startswith('>'):
                ...
                if first_word_without_version in refseq_genbank:
                    new_first_word = refseq_genbank[first_word_without_version]
